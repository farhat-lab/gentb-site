...
parser = argparse.ArgumentParser(description='processes prediction json and var and updates json with other mutations to display in the prediction report, and includes a new category of lineage mutations. Includes a set of drugs and gene names')

parser.add_argument('var_file', help='var file')
parser.add_argument('lineage_snps_file', help='file with snps that are not rcms but lineage specific to discard from WALKER paper')
parser.add_argument('json_file', help='file that contains R prediction and list of important and other mutations')
...
drugs_to_genes = {
                ...
                'INH':set(['katG','promoter-fabG1-inhA','inhA','kasA','promoter-ahpC','promoter-embA-embB']),
...
class Variant:
	...
	def compare_variant(self, variant):
		return (self.gene_name == variant.gene_name) and (self.codon_location == variant.codon_location) ##and (self.AA_change == variant.AA_change) ##will include the AA matching below for coding snps and Indel intergenic/promoters
	def __str__(self):
		if(self.AA_change):
...
drs=['INH','RIF','PZA','EMB','STR','ETH','KAN','CAP','AMK','CIP','LEVO','OFLX','PAS']
   ...
   if(mut):
     ...
     if(type_change_info[0] == 'SNP'):
          if(type_change_info[1] == 'CN'):
                 ...
                 gene_name, codonAA = type_change_info[5], type_change_info[4].replace('.','*')
                 type_change,codon_position = codonAA[0]+codonAA[len(codonAA)-1], codonAA[1:len(codonAA)-1]
                 ...
                 gene_name = 'inter-'+type_change_info[5]+'-'+type_change_info[6]
          ...
          if m:
               	 indel_seq = m.group()
          type_change = codonAA[0]+type_change_info[1][1]+indel_seq.replace('\n','') #e.g dFG or iFGA
          ...
          if m:
               	 indel_seq = m.group()
          type_change = codonAA[0]+type_change_info[1][1]+indel_seq.replace('\n','') #e.g dIG or iDGA
     elif(type_change_info[0] in ['INS','DEL']): #must be indel in promoter or intergenic region
...
for line in open(args.lineage_snps_file,'r').readlines()[1:]:
	...
	if(type_change_info[1] == 'CN'): #not currently used or tested
		...
		gene_name, codonAA = type_change_info[4], type_change_info[3]
		type_change,codon_position = codonAA[0]+codonAA[len(codonAA)-1], codonAA[1:len(codonAA)-1] 
	elif(type_change_info[0] == 'INS' and type_change_info == 'F'): #not currently used or tested
	...
	elif(type_change_info[0] == 'DEL' and type_change_info[1] == 'F'): #not currently used or tested
		gene_name, codon_position = type_change_info[4], type_change_info[3]
		type_change,codon_position = codonAA[0]+codonAA[len(codonAA)-1], codonAA[1:len(codonAA)-1].replace('\n','') 
        ...
        if len(m)>=1:
            for j in range(0,len(m)):
                ...
                if(type_change_info[1] != 'CS'):
                 if(type_change_info[0] == "SNP" and type_change_info[1] in ['CN','CZ']):
                        gene_name, codonAA = type_change_info[5], type_change_info[4]
                        type_change,codon_position = codonAA[0]+codonAA[len(codonAA)-1], codonAA[1:len(codonAA)-1]
                        ...
                        gene_name, codonAA = type_change_info[4], type_change_info[3]
                        type_change,codon_position = codonAA[0]+codonAA[len(codonAA)-1], codonAA[1:len(codonAA)-1]
                 elif(type_change_info[0] in ['INS','DEL'] and type_change_info[1] in ['CF','CD','CI']):
                        gene_name, codon_position = type_change_info[5], type_change_info[4]
                        ...
                        if m:
                                 ...
                                 indel_seq = m.group(2)
                        type_change = codonAA[0]+indel_seq.replace('\n','')
                 test = Variant(gene_name, codon_position, type_change, name=str(var))
                 lineage_test = 0
                 ...
                 if(lineage_test == 0):
                        for imp in impoth_variants_identified:
                                 if(not imp.compare_variant(test)):
                                     impoth_test = 0
                                 elif(imp.AA_change[0:2] not in ['dF','iF'] and imp.AA_change != test.AA_change):
                                     impoth_test = 0
                                 else:
                                     impoth_test = 1
                        if(impoth_test ==0):
